...
@Component({
    ...
    templateUrl: 'caixa-padrao.component.html'
})

export class CaixaPadraoComponent implements OnInit {
    ...
    selectYear() {
        ...
        const listaCaixaPadrao: number[] = [];

        const ano = 2018;
        this.resumoDiarioService.getCaixaPadraoPorAno(ano).subscribe(
            (resp) => {
                ...
                lista.caixaPadraoWrapper.forEach(caixa => {
        ...
        new Chartist.Line('#caixaPadraoChart', dataMonth, options, responsiveOptions);
    }
    ...
    selectMonth() {
        ...
        const ano = 2018;
        const mes = 4;
        this.resumoDiarioService.getCaixaPadraoPorMes(ano, mes).subscribe(
            (resp) => {
